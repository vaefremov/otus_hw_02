...
std::string OTUS::version_str() { return std::string(VERSION_NUMBER); }
        ...
        try {
            ...
            if (v.size() <= column) {
                throw std::runtime_error(
                    "Number of columns is less than the specified column number");
            }
            ip_pool.push_back(OTUS::string_to_ip(v.at(column)));
        } catch (std::exception& ex) {
            std::cerr << "Warning, bad input string " << line_no << "." << column
                      << ": " << line << std::endl;
